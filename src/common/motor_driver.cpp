...
int rotate_to_pos(POS_TYPE want_x, POS_TYPE want_y, u08* bumpers, u08* flags) {
  POS_TYPE diff_x, diff_y, diff_a;
  POS_TYPE ang_to_pos;

  int start = 0;
  do {//rotate to want_x, want_y
    if (start > 32)
      start = 32;
#ifndef TEST_JIZDA
    if (check_travel(bumpers, NULL, NULL, flags) != 0) {
      motor_stop();
      return 1;
    }
#endif
    pos_last = pos_now;
    enc_get(&pos_now);
    diff_x = want_x - pos_now.pos_x;
    diff_y = want_y - pos_now.pos_y;
    ang_to_pos = atan(diff_y / diff_x) * 180.0 / PI;
    if (diff_x < 0)
      ang_to_pos += 180;
    diff_a = ang_to_pos - pos_now.pos_a;
    diff_a = norm_angle(diff_a);
#ifdef VERBOSE
    printf("I want to rotate %f = %i degrees to left\n", diff_a, (int) diff_a);
#endif
    motor_go(0, ((int)diff_a * start) >> 5);
    usleep(pid_loop_usleep);
    start++;
  } while (diff_a > 2 || diff_a < -2);
  return 0;
}
*/
